...
if (!class_exists('Jobsearch_Shortcode_Employers_Frontend')) {
    ...
    class Jobsearch_Shortcode_Employers_Frontend
    {
        ...
        public function jobsearch_employers_content($employer_arg = '')
        {
            ...
            global $wpdb, $post, $jobsearch_form_fields, $jobsearch_search_fields, $pagenow, $jobsearch_plugin_options, $sitepress, $employer_sortin_order;
...
//
            ...
            $employer_sortin_order = $qryvar_employer_sort_type;
            ...
            //var_dump($employer_loop_obj->request);
            ...
            ?>
            <form autocomplete="off" id="jobsearch_employer_frm_<?php echo absint($employer_short_counter); ?>">
                <?php
                //
            ...
            if (isset($_REQUEST['action']) && $pagenow != 'post.php' && strpos($_REQUEST['action'], 'jobsearch_') !== false) {
                die();
            }
        }
        ...
        {
            global $employer_sortin_order;
            $order = $employer_sortin_order != '' ? $employer_sortin_order : 'DESC';
            if (
                !isset($wp_query->query) || $wp_query->is_page || (isset($wp_query->query['post_type']) && $wp_query->query['post_type'] != 'employer')
            ) {
            ...
            $orderby_statement = "cast(postmeta.meta_value as unsigned) DESC, post_date {$order}";
            ...
            if (isset($qrystr) && $qrystr != '') {
                ...
                if (isset($qrystr)) {
                    ...
                    foreach ($qrystr_arr as $qry_var => $qry_val) {
                        ...
                        if (strpos($qry_var, 'nonce') !== false || 'employer_page' == $qry_var || 'lang' == $qry_var || 'page_id' == $qry_var || 'per-page' == $qry_var || 'tab' == $qry_var || 'action' == $qry_var || 'ajax_filter' == $qry_var || 'advanced_search' == $qry_var || 'employer_arg' == $qry_var || 'action' == $qry_var || 'alert-frequency' == $qry_var || 'alerts-name' == $qry_var || 'loc_polygon' == $qry_var || 'alerts-email' == $qry_var || 'loc_polygon_path' == $qry_var)
                            continue;
                        if ($qry_val != '') {
                            if (!is_array($qry_val)) {
