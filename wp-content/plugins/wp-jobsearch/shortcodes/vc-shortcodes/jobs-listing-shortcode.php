...
if (!class_exists('Jobsearch_Shortcode_Jobs_Frontend')) {
    ...
    class Jobsearch_Shortcode_Jobs_Frontend {
        ...
        public function jobs_list_args($atts = array()) {
            ...
            $got_skill_keyword = false;

            $skill_in = '';
            if (isset($_REQUEST['search_title']) && $_REQUEST['search_title'] != '') {
                $title_keyword = jobsearch_esc_html($_REQUEST['search_title']);
                $skill_term = get_term_by('name', $title_keyword, 'skill');
                if (isset($skill_term->slug) && isset($skill_term->count) && $skill_term->count > 0) {
                    $got_skill_keyword = true;
                    $skill_in = $skill_term->slug;
                }
            }
            if (isset($_REQUEST['skill_in']) && $_REQUEST['skill_in'] != '') {
                $skill_in = jobsearch_esc_html($_REQUEST['skill_in']);
            }
            if (isset($_REQUEST['job_skills']) && $_REQUEST['job_skills'] != '') {
                $skill_in = jobsearch_esc_html($_REQUEST['job_skills']);
                $skill_in = explode(',', $skill_in);
            }
            ...
            if ($search_title != '' && $got_skill_keyword === false) {
            ...
            if (isset($_REQUEST['sector_cat']) && $_REQUEST['sector_cat'] != '') {
                ...
                if (!empty($sec_terms_expl)) {
                    if (sizeof($sec_terms_expl) > 1) {
                        $new_terms_arr = array();
                        foreach ($sec_terms_expl as $sec_term_expl) {
                            $get_term_obj = get_term_by('slug', $sec_term_expl, 'sector');
                            if (isset($get_term_obj->parent) && $get_term_obj->parent > 0) {
                                $new_terms_arr[] = $get_term_obj->slug;
                            }
                        }
                    }
                    if (isset($new_terms_arr) && !empty($new_terms_arr)) {
                        $sec_terms_arr = $new_terms_arr;
                    } else {
                        foreach ($sec_terms_expl as $sec_term_expl) {
        ...
        public function jobsearch_jobs_content($job_arg = '') {
            ...
            if (isset($_REQUEST['job_arg']) && $_REQUEST['job_arg']) {
                ...
                $job_arg_obj = json_decode($job_arg);
                if ($job_arg_obj == null) {
                    $smjob_arg = stripslashes(html_entity_decode($_REQUEST['smjob_arg']));
                    $job_arg_obj = json_decode($smjob_arg);
                }
                $job_arg = $this->toArray($job_arg_obj);
            ...
            //
            if (!isset($atts) || (isset($atts) && empty($atts))) {
                $atts['job_cat'] = '';
                $atts['display_per_page'] = 'no';
                $atts['job_alerts_top'] = 'no';
                $atts['job_alerts_bottom'] = 'yes';
                $atts['featured_only'] = 'no';
                $atts['job_alerts'] = 'no';
                $atts['job_custom_fields_switch'] = 'yes';
                $atts['quick_apply_job'] = 'off';
                $atts['job_loc_listing'] = 'state';
                $atts['job_deadline_switch'] = 'no';
                $atts['job_filters'] = 'yes';
                $atts['job_filters_count'] = 'yes';
                $atts['job_filters_sortby'] = 'default';
                $atts['job_filters_keyword'] = 'no';
                $atts['job_filters_loc'] = 'no';
                $atts['job_filters_loc_collapse'] = 'no';
                $atts['job_filters_loc_view'] = 'dropdowns';
                $atts['job_filters_date'] = 'no';
                $atts['job_filters_date_collapse'] = 'no';
                $atts['job_filters_type'] = 'yes';
                $atts['job_filters_type_collapse'] = 'no';
                $atts['job_filters_sector'] = 'no';
                $atts['job_filters_sector_collapse'] = 'no';
                $atts['job_top_map'] = 'yes';
                $atts['job_top_map_height'] = '450';
                $atts['job_top_map_zoom'] = '0';
                $atts['job_feat_jobs_top'] = 'yes';
                $atts['num_of_feat_jobs'] = '5';
                $atts['job_top_search'] = 'yes';
                $atts['job_top_search_view'] = 'simple';
                $atts['top_search_title'] = 'yes';
                $atts['top_search_location'] = 'yes';
                $atts['top_search_sector'] = 'yes';
                $atts['top_search_autofill'] = 'yes';
                $atts['job_elem_custom_fields'] = '3841353,1684867,8303213';
                $atts['job_sort_by'] = 'yes';
                $atts['job_rss_feed'] = 'no';
                $atts['jobs_emp_base'] = 'no';
                $atts['jobs_emp_base_id'] = '';
                $atts['job_excerpt'] = '20';
                $atts['job_order'] = 'DESC';
                $atts['job_orderby'] = 'ID';
                $atts['job_pagination'] = 'yes';
                $atts['job_per_page'] = '15';
                $atts['job_view'] = 'view-default';
                $atts['job_type'] = '';
            }
            ...
            //if (isset($_GET['get_query_requst'])) {
            ...
            ?>
            <form autocomplete="off" id="jobsearch_job_frm_<?php echo absint($job_short_counter); ?>">
                <?php
                ...
                <div style="display:none" id='job_small_arg<?php echo absint($job_short_counter); ?>'>
                    <?php
                    $jobs_arggs = apply_filters('jobsearch_injobsh_ajax_args_list', $job_arg);
                    $jobs_arggs_small = $jobs_arggs;
                    unset($jobs_arggs_small['atts']);
                    echo json_encode($jobs_arggs_small);
                    ?>
                </div>
                <?php
                if ($atts['job_top_search'] != 'no') {
                    ...
                    $get_loc_radius = $top_search_def_radius;
                    if (isset($_REQUEST['loc_radius'])) {
                        $get_loc_radius = $_REQUEST['loc_radius'];
                    }

                    ob_start();
                    if ($job_view == 'view-listing-10') {
                        ?>
                        <div class="careerfy-main-section careerfy-search-twentyone-full careerfy-listin-tsrch jobsearch-top-searchbar jobsearch-typo-wrap <?php echo apply_filters('jobsearch_joblistn_thesrch_mainclass', $search_main_class) ?> <?php echo $adv_search_on ?>">
                            <div class="row">
                                <div class="col-md-12">
                                        ...
                                        <div class="careerfy-search-twentyone-grid">
                                            ...
                                            if ($top_search_title != 'no') {
                                                ...
                                                <div class="advserch-field">
                                                    <div class="<?php echo($top_search_autofill != 'no' ? 'jobsearch-sugges-search' : '') ?> advsrch-keyword-field">
                                            ...
                                            <div class="advserch-field">
                                                ...
                                                <button type="submit" id="jobsearch-jobadvserach-submit" class="advsrch-act-btn"><i class="careerfy-icon careerfy-rockit-fill"></i> <?php esc_html_e("Looking for a dream job", 'careerfy-frame') ?></button>
                                            </div>
                                        </div>
                                        <?php
                                        if ($top_serch_style == 'advance') {
                ...
                <div class="jobsearch-row">
                    ...
                    <div class="<?php echo esc_html($content_columns); ?>">
                        ...
                        <div class="wp-jobsearch-job-content jobsearch-listing-maincon wp-jobsearch-dev-job-content"
                            ...
                            if (isset($_GET['shw_args'])) {
                                ?>
                                <input type="hidden" name="shw_args" value="shw_args">
                                <?php
                            }
        ...
        public function location_radius_filter_ids() {
            ...
            if (isset($_REQUEST['location'])) {
                $exist_loc = true;
                $location_str = $_REQUEST['location'];
            } else if (isset($_REQUEST['location_location3'])) {
                $exist_loc = true;
                $location_str = $_REQUEST['location_location3'];
            } else if (isset($_REQUEST['location_location2'])) {
                $exist_loc = true;
                $location_str = $_REQUEST['location_location2'];
            } else if (isset($_REQUEST['location_location1'])) {
                $exist_loc = true;
                $location_str = $_REQUEST['location_location1'];
            }
            
            if (isset($_REQUEST['loc_radius']) && $_REQUEST['loc_radius'] > 0 && $exist_loc) {
                ...
                $jobsearch_loc_address = $location_str . ($country_name != '' ? ' ' . $country_name : '');
                $radius = jobsearch_esc_html($_REQUEST['loc_radius']);
            ...
            if (isset($qrystr) && $qrystr != '') {
                ...
                if (isset($qrystr)) {
                    ...
                    foreach ($qrystr_arr as $qry_var => $qry_val) {
                        ...
                        if (strpos($qry_var, 'nonce') !== false || 'job_page' == $qry_var || 'woof_parse_query' == $qry_var || 'ao_noptimize' == $qry_var || 'page_id' == $qry_var || 'lang' == $qry_var || 'tab' == $qry_var || 'per-page' == $qry_var || 'action' == $qry_var || 'ajax_filter' == $qry_var || 'advanced_search' == $qry_var || 'job_arg' == $qry_var || 'smjob_arg' == $qry_var || 'action' == $qry_var || 'alert-frequency' == $qry_var || 'alerts-name' == $qry_var || 'loc_polygon' == $qry_var || 'alerts-email' == $qry_var || 'loc_polygon_path' == $qry_var || 'alert_frequency' == $qry_var || 'sh_globrnd_id' == $qry_var || 'job_shatts_str' == $qry_var) {
                            continue;
                        }
                        ...
                        if ('job_salary_type' == $qry_var && !empty($job_salary_types)) {
                            if ($qry_val != '') {
                                ...
                                $keyword_html .= '<a href="' . wp_kses(jobsearch_remove_qrystr_extra_var($qrystr, $qry_var), array()) . '" title="' . ($qry_var_str) . '">' . $qry_var_str . ': ' . $salary_type_val_str . ' <i class="fa fa-close"></i></a>';
                                $keyword_html .= '</li>';
                            }
                        } else {
                            if ($qry_val != '') {
                                ...
                                if (is_array($qry_val)) {
                                    foreach ($qry_val as $qry_val_var => $qry_val_value) {
                                        if ($qry_val_value != '') {
                                            ...
                                            $keyword_html .= '<a href="' . wp_kses(jobsearch_remove_qrystr_extra_var($qrystr1, $qry_var), array()) . '" title="' . $qry_var_str . '">' . $qry_var_str . ': ' . jobsearch_esc_html(stripslashes($qry_val_str)) . ' <i class="fa fa-close"></i></a>';
                                            $keyword_html .= '</li>';
                                        }
                                    }
                                } else {
                                    ...
                                    $keyword_html .= '<a href="' . wp_kses(jobsearch_remove_qrystr_extra_var($qrystr, $qry_var), array()) . '" title="' . $var_title . '">' . $var_title . ': ' . jobsearch_esc_html(stripslashes($qry_val_str)) . ' <i class="fa fa-close"></i></a>';
                                    $keyword_html .= '</li>';
                                }
                            }
                        }
