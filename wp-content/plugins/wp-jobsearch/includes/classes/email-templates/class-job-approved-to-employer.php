 ...
 * do_action('jobsearch_job_approved_to_employer', $user_data, $post_id);
 * 
 */
if (!class_exists('jobsearch_job_approved_to_employer_template')) {

    class jobsearch_job_approved_to_employer_template {
        ...
        public function __construct() {
            ...
            add_action('init', array($this, 'jobsearch_job_approved_to_employer_template_init'), 1, 0);
            add_filter('jobsearch_job_approved_to_employer_filter', array($this, 'jobsearch_job_approved_to_employer_filter_callback'), 1, 4);
            add_filter('jobsearch_email_template_settings', array($this, 'template_settings_callback'), 12, 1);
            add_action('jobsearch_job_approved_to_employer', array($this, 'jobsearch_job_approved_to_employer_callback'), 10, 2);
        }

        public function jobsearch_job_approved_to_employer_template_init() {
            $this->user = array();
            $this->rand = rand(0, 99999);
            $this->group = 'job';
            $this->type = 'job_approved_to_employer';
            $this->filter = 'job_approved_to_employer';
            $this->email_template_db_id = 'job_approved_to_employer';
            $this->switch_label = esc_html__('New job approved by admin send it to employer', 'wp-jobsearch');
            $this->default_subject = esc_html__('New job approved by admin send it to employer', 'wp-jobsearch');
            $this->default_recipients = '';
            $default_content = esc_html__('Default content', 'wp-jobsearch');
            $default_content = apply_filters('jobsearch_job_approved_to_employer_filter', $default_content, 'html', 'job-approved-to-employer', 'email-templates');
            $this->default_content = $default_content;
            $this->email_template_prefix = 'job_approved_to_employer';
            ...
            $this->codes = array(
                ...
                array(
                    'var' => '{edit_job_url}',
                    'display_text' => esc_html__('Edit job URL', 'wp-jobsearch'),
                    'function_callback' => array($this, 'get_job_edit_url'),
                ),
                array(
                    'var' => '{job_jobtype}',
        ...
        public function jobsearch_job_approved_to_employer_callback($user = '', $job_id = '') {
            global $sitepress;
            $lang_code = '';
            ...
            if (isset($template['switch']) && $template['switch'] == 1) {
                ...
                $to_send_email = apply_filters('jobsearch_emp_tosend_email_b4_recipient', $to_send_email, $job_id, $this_temp_type);
                ...
                jobsearch_job_approved_to_employer_template::$is_email_sent1 = $this->is_email_sent;
            }
        }
        ...
        public function jobsearch_job_approved_to_employer_filter_callback($html, $slug = '', $name = '', $ext_template = '') {
            ob_start();
            $html = '';
            $template = '';
            if ($ext_template != '') {
        ...
        public function template_settings_callback($email_template_options) {
            ...
            $email_template_options['job_approved_to_employer']['rand'] = $this->rand;
            $email_template_options['job_approved_to_employer']['email_template_prefix'] = $this->email_template_prefix;
            $email_template_options['job_approved_to_employer']['email_template_group'] = $this->email_template_group;
            $email_template_options['job_approved_to_employer']['default_var'] = $this->default_var;
        ...
        public function get_job_edit_url() {
            ...
            $job_url = add_query_arg(array('tab' => 'user-job', 'job_id' => $job_id, 'action' => 'update'), $page_url);
            return $job_url;
        }
    ...
    new jobsearch_job_approved_to_employer_template();
}