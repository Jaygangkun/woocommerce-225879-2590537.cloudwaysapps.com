...
if (!class_exists('jobsearch_location_ajax')) {
    ...
    class jobsearch_location_ajax
    {
        ...
        public function getExistingLocationDetail($param_1, $param_2, $loc_type)
        {
            ...
            switch ($loc_type) {
                case "country":
                    $this->query = "SELECT * FROM `{$wpdb->base_prefix}jobsearch_countries` WHERE name = '" . ($param_1) . "' and code = '" . $param_2 . "'";
                    return $wpdb->get_results($this->query);
                    break;
                case "state":
                    $this->query = "SELECT * FROM `{$wpdb->base_prefix}jobsearch_states` WHERE state_name = '" . ($param_1) . "' and cntry_id = '" . $param_2 . "'";
                    return $wpdb->get_results($this->query);
                    break;
                case "city":
                    $this->query = "SELECT * FROM `{$wpdb->base_prefix}jobsearch_cities` WHERE city_name = '" . ($param_1) . "' and state_id = '" . $param_2 . "'";
                    return $wpdb->get_results($this->query);
                    break;
            }
        }
