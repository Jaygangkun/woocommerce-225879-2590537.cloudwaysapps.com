...
// hook things up
    ...
    static function jobsearch_custom_field_html_callback($return = false) {
        ...
        $html .= apply_filters('jobsearch_custom_field_actions_bkajax_html', '', $li_rand_id, $global_custom_field_counter, $fieldtype);
        $html .= '</li>';
    ...
    public function jobsearch_custom_fields_save_callback() {
        if (current_user_can('administrator') && check_ajax_referer('jobsearch_cus_fields_settings_nonce', '_wpnonce_cus_fields_settings', false)) {
            $post_data = $_POST;
            $post_data = empty($post_data) ? array() : $post_data;
            $heading_counter = $textarea_counter = $dropdown_counter = $checkbox_counter = $date_counter = $email_counter = $upload_counter = $text_counter = $linkurl_counter = $video_counter = $number_counter = $dependent_dropdown_counter = $dependent_fields_counter = $range_counter = $salary_counter = $company_benefits = $start_month_counter = $error = 0;
                ...
                foreach ($post_data['jobsearch-custom-fields-type'] as $custom_fields_type) {
                    ...
                    switch ($custom_fields_type) {
                        ...
                        case('company_benefits'):
                            ...
                            break;
                        case('start_month'):
                            $custom_field_array = $this->jobsearch_custom_field_ready_array($start_month_counter, $custom_fields_type, $custom_field_array);
                            $start_month_counter ++;
                            break;
                    }
    ...
    public function jobsearch_custom_field_ready_array($array_counter = 0, $field_type = '', $custom_field_element_array = array()) {
        ...
        foreach ($possible_element_fields as $field) {
            if (isset($_POST["jobsearch-custom-fields-{$field_type}"][$field][$array_counter])) {
                $field_label = isset($_POST["jobsearch-custom-fields-{$field_type}"]['label'][$array_counter]) && $_POST["jobsearch-custom-fields-{$field_type}"]['label'][$array_counter] != '' ? stripslashes($_POST["jobsearch-custom-fields-{$field_type}"]['label'][$array_counter]) : 'Field Name';
                if ($field == 'name') {
